        ...
        for r=1:length(param.r)
            ...
            for i=1:size(a.v.merge,1)
                for j=1:size(a.v.merge,2)
                    plot3(param.totalPathLength/a.v.merge(i,j).sum, param.totalPathLength/a.v.split(i,j).sum, a.v.general(i,j).maxNrObjects, marker{mod(i,13) + 1}, 'MarkerEdgeColor', autoKLEE_colormap(j,:), 'MarkerSize', markerSize(r));
                    hold on;
...
%                 b = load([param.dataFolder param.outputSubfolder param.affMaps(map).name '/seg' num2str(r) '-' num2str(algo) '.mat']);
                for i=1:size(a.v.merge,1)
                    for j=1:size(a.v.merge,2)
                        plot3(param.r(r), param.pR{map,algo}{1}(i), param.pR{map,algo}{2}(j), marker{mod(i,13) + 1}, 'MarkerEdgeColor', autoKLEE_colormap(j,:), 'MarkerSize', markerSize(r));
                        hold on;
                    end
                end
            end
