...
/* Copyright 2013, 2014 Max Planck Institute of Neurobiology, Martinsried                                */
/* Version 0.12   Martin Zauser                                                                          */
/* ***************************************************************************************************** */
...
#define VERSION       "0.12"
#define VERSION_DATE  "03.03.2014"
...
#define MAX_NUMBER_OF_ATTRIBUTES       20
...
void mexFunction (int nlhs , mxArray *plhs[], int nrhs , const mxArray *prhs[]) {
    ...
    const char *nmlNodeAttributes[] = { "id", "radius", "x", "y", "z", "comment" };
    ...
    if (nrhs > 3) {
        mexErrMsgIdAndTxt("Braintracing:parseNML:nrhs", "Only three parameters allowed: filename[character], keepNodeAsStruct[0/1] and nodesAsStruct is cell[0/1].");
        return;
    }
    if (nrhs < 1) {
        /* ************ */
    ...
    if (nrhs >= 3) {
        ...
        if (!mxIsClass(prhs[2], "double")) {
            mexErrMsgIdAndTxt("Braintracing:parseNML:notString", "Input parameter 'nodesAsStruct is cell' must be 0 or 1.");
            return;
        }
        /* get parameter */
