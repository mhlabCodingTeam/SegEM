/* RANDRMEXMASKGREATERZERO.CPP Use to calculate the crucial stage of the rand error in Matlab
...
Only count the faulty connections, which are connected to a non-zero part in seg. */
				...
				for (ii = max(0, i - rMax); ii < min(N1, i + rMax + 1); ii++)
				{
					for (jj = max(0, j - rMax); jj < min(M1, j + rMax + 1); jj++)
					{
						for (kk = max(0, k - rMax); kk < min(P1, k + rMax + 1); kk++)
						{
							...
							/* Pick the correct range in distance and for which at least one of the partners in the pair is nonzero in the segmentation. */
							if ((sqrt(dist) > R1[0]) && (sqrt(dist) <= R2[0]) && ((thisSeg != 0) || (seg[ii + N1 * (jj + M1 * kk)] != 0)) && ((thisGT != 0) || (gT[ii + N1 * (jj + M1 * kk)] != 0)))
							{
